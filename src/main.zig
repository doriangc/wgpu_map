...
pub fn main() anyerror!void {
    ...
    rl.setConfigFlags(.{ .msaa_4x_hint = false, .window_highdpi = true, .window_resizable = true });
    // const screenHeight: i32 = @intFromFloat(@as(f64, @floatFromInt(rl.getMonitorHeight(0))));
    // const screenWidth: i32 = @intFromFloat(@as(f64, @floatFromInt(rl.getMonitorWidth(0))));

    rl.initWindow(0, 0, "TerraCity Debugger");
    ...
    rl.setTargetFPS(rl.getMonitorRefreshRate(rl.getCurrentMonitor())); // Set our game to run at the monitor's refresh rate
    // rl.setTargetFPS(60);
    //--------------------------------------------------------------------------------------
